...
def analyze_trace(bundle, group_id, series_id, trace_name, axs_start_idx, axs, file_name):
    ...
    try:
        ...
        # ======================================================
        # Export baseline + fits for browser
        # ======================================================
        ...
        fit_times = fit_plot_x.tolist()

        # ---- 1-exp fit ----
        if not np.isnan(A_fit):
            fit1_points = list(zip(fit_times, fit_plot_y.tolist()))
        else:
            fit1_points = []
        ...
        # ---- 1-expY fit ----
        if not np.isnan(A_fit_y):
            fit2_points = list(zip(fit_times, fit_plot_y_expY.tolist()))
        else:
            fit2_points = []
        ...
        # ---- 2-exp fit ----
        if not np.isnan(A_fit_2exp):
            fit3_points = list(zip(fit_times, fit_plot_y_2exp.tolist()))
        else:
            fit3_points = []
        ...
        if t0 is not None and t1 is not None:
            ...
            if np.any(current_mask):
                ...
                axs[axs_start_idx + 4].set_title("Calcium current – No Data")
        else:
            ca_current_trace = None
            ca_current_time  = None
...
def analyze_aps_trace(bundle, group_id, series_id, trace_name, axs_start_idx, axs, file_name):
    ...
    try:
        ...
        if do_plot:
            ...
            if not np.isnan(A1):
                ax(1).plot(fit_plot_x, fit1, 'r--', label=f"1-exp\nA={A1:.2f} pF, τ={tau1:.2f} s")
        ...
        # Export for browser
        baseline_points = list(zip([time[0], time[-1]], [baseline_fit_line[0], baseline_fit_line[-1]]))
        processed_data = {
...
def analyze_aps_average(time_rel, cm_bs, axs_start_idx, axs, trace_name="aps Average",
    ...
    try:
        ...
        axs[axs_start_idx + 1].plot(time_rel, cm_bs, label="Cm average")
        if not np.isnan(A1):
